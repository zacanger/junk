...
use zterm_terminal::ansi;
use zterm_terminal::config::MockConfig;
use zterm_terminal::event::{Event, EventListener};
use zterm_terminal::grid::{Dimensions, Grid};
use zterm_terminal::index::{Column, Line};
use zterm_terminal::term::cell::Cell;
use zterm_terminal::term::{SizeInfo, Term};
...
fn ref_test(dir: &Path) {
    let recording = read_u8(dir.join("zterm.recording"));
