...
const ZTERM_LOG_ENV: &str = "ZTERM_LOG";
/// List of targets which will be logged by Zterm.
const ALLOWED_TARGETS: [&str; 4] =
    ["zterm_terminal", "zterm_config_derive", "zterm", "crossfont"];
...
impl Logger {
    ...
    fn message_bar_log(&self, record: &log::Record<'_>, logfile_path: &str) {
        ...
        let env_var = format!("${}", ZTERM_LOG_ENV);
...
impl OnDemandLogFile {
    fn new() -> Self {
        ...
        path.push(format!("Zterm-{}.log", process::id()));

        // Set log path as an environment variable.
        env::set_var(ZTERM_LOG_ENV, path.as_os_str());
