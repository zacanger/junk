//! Zterm - The GPU Enhanced Terminal.
...
use zterm_terminal::event_loop::{self, EventLoop, Msg};
use zterm_terminal::grid::Dimensions;
use zterm_terminal::sync::FairMutex;
use zterm_terminal::term::Term;
...
fn main() {
    ...
    if let Err(err) = run(window_event_loop, config, options) {
        error!("Zterm encountered an unrecoverable error:\n\n\t{}\n", err);
        std::process::exit(1);
    }
