;(function () {
  ...
  showdown.helper.matchRecursiveRegExp = function (str, left, right, flags) {
    'use strict'
    var f = flags || '', g = f.indexOf('g') > -1, x = new RegExp(left + '|' + right, f), l = new RegExp(left, f.replace(/g/g, '')), a = [], t, s, m, start, end
    do {
      t = 0
      while (m = x.exec(str)) {
        if (l.test(m[0])) {
  ...
  showdown.Converter = function (converterOptions) {
    'use strict'
    var options = {}, langExtensions = [], outputModifiers = [], listeners = {}
    _constructor()
  ...
  showdown.subParser('headers', function (text, options, globals) {
    ...
    var prefixHeader = options.prefixHeaderId, headerLevelStart = isNaN(parseInt(options.headerLevelStart)) ? 1 : parseInt(options.headerLevelStart), setextRegexH1 = options.smoothLivePreview ? /^(.+)[ \t]*\n={2,}[ \t]*\n+/gm : /^(.+)[ \t]*\n=+[ \t]*\n+/gm, setextRegexH2 = options.smoothLivePreview ? /^(.+)[ \t]*\n-{2,}[ \t]*\n+/gm : /^(.+)[ \t]*\n-+[ \t]*\n+/gm
  ...
  showdown.subParser('tables', function (text, options, globals) {
    ...
    var table = function () {
      ...
      tables.thead = function () {
        var out, hs = [].slice.apply(arguments[0]), style = [].slice.apply(arguments[1])
        out = '<thead>\n'
        out += '<tr>\n'
      ...
      tables.tr = function () {
        var out, cs = [].slice.apply(arguments[0]), style = [].slice.apply(arguments[1])
        out = '<tr>\n'
      ...
      filter = function (text) {
        ...
        for (i; i < lines.length; i += 1) {
          ...
          if (line.trim().match(/^[|].*[|]$/)) {
            line = line.trim()
            var tbl = [], align = lines[i + 1].trim(), styles = [], j = 0
            if (align.match(/^[|][-=|: ]+[|]$/)) {
