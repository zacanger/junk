...
#define LASSERT_TYPE(func, args, index, expect) \
  LASSERT( \
    ...
    "'%s' got wrong type for argument %i. %s is not %s", \
    func, \
    index, \
...
#define LASSERT_NUM(func, args, num) \
  LASSERT( \
    args, \
    args->count == num, \
    "'%s' got wrong number of arguments. %i is not %i", \
    func, \
    args->count, \
    num \
  );
...
lval* builtin_var(lenv* e, lval* a, char* func) {
  ...
  LASSERT(
    ...
    "%s got too many symbol arguments"
    "%i is not %i",
    func,
    syms->count,
    a->count-1
...
lval* builtin_load(lenv* e, lval* a) {
  ...
  if (f == NULL) {
    lval* err = lval_err("failed to load %s", a->cell[0]->str);
    lval_del(a);
    return err;
  }
